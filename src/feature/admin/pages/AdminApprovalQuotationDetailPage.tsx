...
export default function AdminApprovalQuotationDetailPage() {
  ...
  const handleStartReview = async () => { if (!id) return; try { setStartingReview(true); setError(null); await quotationService.startStaffReview(id); await fetchDetail(); } catch (err: any) { console.error(err); setError(err?.response?.data?.msg || "Không thể bắt đầu review quotation."); } finally { setStartingReview(false); } };
  const handleSendToAdmin = async () => { if (!id) return; try { setSendingAdmin(true); setError(null); await quotationService.sendStaffQuotationToAdmin(id, sendAdminMessage.trim() || undefined); setSendAdminMessage(""); await fetchDetail(); } catch (err: any) { console.error(err); setError(err?.response?.data?.msg || "Không thể gửi quotation cho admin."); } finally { setSendingAdmin(false); } };
  const handleApprove = async () => { if (!id) return; try { setSubmittingApprove(true); setError(null); await quotationService.approveAdminQuotation(id, { message: decisionMessage.trim() || undefined }); setDecisionMessage(""); await fetchDetail(); } catch (err: any) { console.error(err); setError(err?.response?.data?.msg || "Không thể approve quotation."); } finally { setSubmittingApprove(false); } };
  const handleReject = async () => { if (!id) return; try { setSubmittingReject(true); setError(null); await quotationService.rejectAdminQuotation(id, { message: decisionMessage.trim() || undefined }); setDecisionMessage(""); await fetchDetail(); } catch (err: any) { console.error(err); setError(err?.response?.data?.msg || "Không thể reject quotation."); } finally { setSubmittingReject(false); } };
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Trang admin theo dõi toàn bộ luồng review phí và phê duyệt quotation.</p>
        </div>
      ...
      {(canStartReview || canSendAdmin) && <div className="bg-white rounded-2xl border border-violet-200 p-4 shadow-sm"><p className="text-sm font-semibold text-violet-800">Thao tác review</p><div className="mt-2 flex flex-col md:flex-row gap-2"><input value={sendAdminMessage} onChange={(e) => setSendAdminMessage(e.target.value)} placeholder="Nhập ghi chú nội bộ hoặc gửi admin (tuỳ chọn)" className="flex-1 rounded-xl border border-violet-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-200" />{canStartReview && <button type="button" onClick={handleStartReview} disabled={startingReview} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#7a160e] px-4 py-2 text-xs font-semibold text-white hover:bg-[#5c0f09] disabled:opacity-60"><PlayCircle className="h-4 w-4" />{startingReview ? "Đang bắt đầu review..." : "Bắt đầu review"}</button>}{canSendAdmin && <button type="button" onClick={handleSendToAdmin} disabled={sendingAdmin} className="inline-flex items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 py-2 text-xs font-semibold text-white hover:bg-violet-700 disabled:opacity-60"><Send className="h-4 w-4" />{sendingAdmin ? "Đang gửi..." : "Gửi admin"}</button>}</div></div>}
      {canDecide && <div className="bg-white rounded-2xl border border-violet-200 p-4 shadow-sm"><p className="text-sm font-semibold text-violet-800">Quyết định của admin</p><textarea value={decisionMessage} onChange={(e) => setDecisionMessage(e.target.value)} placeholder="Nhập ghi chú gửi khách/staff (tuỳ chọn)" rows={3} className="mt-2 w-full rounded-xl border border-violet-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-200" /><div className="mt-3 flex items-center gap-2"><button type="button" onClick={handleApprove} disabled={submittingApprove || submittingReject} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-xs font-semibold text-white hover:bg-emerald-700 disabled:opacity-60"><CheckCircle2 className="h-4 w-4" />{submittingApprove ? "Đang duyệt..." : "Approve gửi khách"}</button><button type="button" onClick={handleReject} disabled={submittingApprove || submittingReject} className="inline-flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2 text-xs font-semibold text-white hover:bg-red-700 disabled:opacity-60"><XCircle className="h-4 w-4" />{submittingReject ? "Đang từ chối..." : "Reject trả staff"}</button></div></div>}
      {error && <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-600">{error}</div>}

      {loading ? <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center text-sm text-gray-500 shadow-sm">Đang tải dữ liệu...</div> : !detail ? <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center text-sm text-gray-500 shadow-sm">Không tìm thấy quotation.</div> : <>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm"><p className="text-xs text-gray-500">Loại quotation</p><p className="text-lg font-semibold text-[#7a160e] mt-1">{detail.quotationType}</p></div>
          ...
          <div className="space-y-4 self-start">
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-3 text-sm text-gray-700">
                ...
                <p>Số điện thoại: <span className="font-semibold">{detail.phone || "N/A"}</span></p><p>Địa chỉ: <span className="font-semibold">{detail.address || "N/A"}</span></p><p>Account ID: <span className="font-semibold">{detail.accountId ?? "-"}</span></p><p>Order ID: <span className="font-semibold">{detail.orderId ?? "-"}</span></p><p>Revision: <span className="font-semibold">{detail.revision}</span></p><p>Staff reviewer: <span className="font-semibold">{detail.staffReviewerId ?? "-"}</span></p><p>Admin reviewer: <span className="font-semibold">{detail.adminReviewerId ?? "-"}</span></p>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-100 text-xs text-gray-600 space-y-2">
                <div className="flex items-center gap-2"><Calendar className="h-4 w-4" />Ngày tạo: {formatDate(detail.requestDate)}</div><div className="flex items-center gap-2"><Clock className="h-4 w-4" />Ngày submit: {formatDate(detail.submittedAt)}</div><div className="flex items-center gap-2"><Clock className="h-4 w-4" />Staff review: {formatDate(detail.staffReviewedAt)}</div><div className="flex items-center gap-2"><Clock className="h-4 w-4" />Admin review: {formatDate(detail.adminReviewedAt)}</div><div className="flex items-center gap-2"><Clock className="h-4 w-4" />Khách phản hồi: {formatDate(detail.customerRespondedAt)}</div>
              </div>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
              <h3 className="text-sm font-semibold uppercase tracking-wider text-[#7a160e]/80">Tổng hợp giá</h3>
              <div className="mt-4 space-y-2 text-sm"><p>Tổng gốc: <span className="font-semibold">{formatMoney(detail.totalOriginal)}</span></p><p>Tổng giảm trừ: <span className="font-semibold text-emerald-700">{formatMoney(detail.totalSubtract)}</span></p><p>Tổng cộng thêm: <span className="font-semibold text-rose-700">{formatMoney(detail.totalAdd)}</span></p><p>Discount amount: <span className="font-semibold">{formatMoney(detail.totalDiscountAmount)}</span></p><p className="pt-2 border-t border-gray-100">Sau điều chỉnh: <span className="font-semibold text-[#7a160e]">{formatMoney(detail.totalAfterDiscount)}</span></p></div>
